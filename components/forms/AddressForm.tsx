...
export default function AddressForm({ cart }: { cart: OrderFragment }) {
    const {
        register,
        handleSubmit,
    ...
    const onSubmit = async (address: AddressType) => {
        await registerGuestCartAddress(null, address);
    };
            ...
            {!isSubmitting && (
                <Form className="border-0 bg-light p-3" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="d-flex flex-column flex-sm-row justify-content-end">
                        <CoreButton type="submit" text="Proceed" icon={false} variant={CoreButtontype.Primary} />
                    </div>
                </Form>
            )}
        </div>
